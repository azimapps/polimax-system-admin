...
import { useRevertMaterial, useGetMaterialHistory } from 'src/hooks/use-materials';
...
export function MaterialHistoryDialog({ open, onClose, materialId }: Props) {
    const { t } = useTranslate('material');
    const { data: history = [], isLoading } = useGetMaterialHistory(materialId);
    const { mutateAsync: revertMaterial } = useRevertMaterial(materialId);
    ...
    const handleConfirmRevert = useCallback(async () => {
        if (selectedVersion) {
            await revertMaterial(selectedVersion);
            confirmDialog.onFalse();
            setSelectedVersion(undefined);
            onClose();
        }
    }, [selectedVersion, revertMaterial, confirmDialog, onClose]);
