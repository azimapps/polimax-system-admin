...
import { useRevertPartner, useGetPartnerHistory } from 'src/hooks/use-partners';
...
export function PartnerHistoryDialog({ open, onClose, partnerId }: Props) {
    const { t } = useTranslate('partner');
    const { data: history = [], isLoading } = useGetPartnerHistory(partnerId);
    const { mutateAsync: revertPartner } = useRevertPartner(partnerId);
    ...
    const handleConfirmRevert = useCallback(async () => {
        if (selectedVersion) {
            await revertPartner(selectedVersion);
            confirmDialog.onFalse();
            setSelectedVersion(undefined);
            onClose();
        }
    }, [selectedVersion, revertPartner, confirmDialog, onClose]);
