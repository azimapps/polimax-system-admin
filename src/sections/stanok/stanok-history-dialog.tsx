...
import { useRevertStanok, useGetStanokHistory } from 'src/hooks/use-stanok';
...
export function StanokHistoryDialog({ open, onClose, stanokId }: Props) {
    const { t } = useTranslate('stanok');
    const { data: history = [], isLoading } = useGetStanokHistory(stanokId);
    const { mutateAsync: revertStanok } = useRevertStanok(stanokId);
    ...
    const handleConfirmRevert = useCallback(async () => {
        if (selectedVersion) {
            await revertStanok(selectedVersion);
            confirmDialog.onFalse();
            setSelectedVersion(undefined);
            onClose();
        }
    }, [selectedVersion, revertStanok, confirmDialog, onClose]);
                            ...
                            <Table>
                                <TableHead>
                                    <TableRow>
                                        ...
                                        <TableCell>{t('form.country_code')}</TableCell>
